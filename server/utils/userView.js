...
// Никнейм (utils/nickname.js) — под ним человека видно везде. Имя (login)
// пока не показывается; оно — запасной путь, пока ник не выдан (до первого
// запуска с никами), дальше — начало почты. Без всего — хвост id: имя
...
//
// Выборки, чей результат идёт сюда, должны брать поле nickname.
function displayName(user) {
  return user.nickname || user.login || (user.email && user.email.split('@')[0]) || '#' + String(user._id).slice(-6);
}
