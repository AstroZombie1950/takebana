...
const ACTIONS = {
  ...
  // Видео галереи на своей странице (routes/watch.js). Загрузка и удаление
  // владельцем с профиля — profile.gallery.*.
  'video.edit': 'Правка видео',
  'video.delete': 'Удаление видео',
  'video.comment': 'Комментарий к видео',
  'video.uncomment': 'Удаление комментария к видео',
  'photo.edit': 'Правка подписи фото',
  'photo.delete': 'Удаление фото',
  'photo.comment': 'Комментарий к фото',
  'photo.uncomment': 'Удаление комментария к фото',

  // Переписка и медиа. Пишем сюда не только отказы, но и удачи по тому,
  // что идёт долго и через чужие руки (пережатие, выгрузка в Bunny):
  // без удач в журнале не видно доли отказов, а без неё нельзя понять,
  // «сломалось у всех» или «не повезло одному». Обычный текст сообщения
  // удачей не пишем — это самый частый запрос сайта, и журнал забился бы им.
  'msg.send': 'Сообщение в переписке',
  'msg.attach': 'Вложение в переписку',
  'msg.forward': 'Пересылка сообщений',
  'media.encode': 'Пережатие видео',
  'media.store': 'Выгрузка файла в хранилище',
