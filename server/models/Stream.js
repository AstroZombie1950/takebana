...
const StreamSchema = new mongoose.Schema({
  ...
  // Веб-эфир с телефона, который держат вертикально: выход Daily собирается
  // в кадр 720×1280, а не 1280×720 с чёрными полосами по бокам. Решается на
  // первом выходе в эфир и до конца не меняется: куски записи склеиваются
  // копией (utils/recording.js), и размер кадра у них должен совпадать.
  portrait: {
    type: Boolean,
    default: null
  },
  // Когда подписчикам в последний раз сказали, что эфир начался
  // (utils/liveNotify.js). Не память ради памяти: эфир с OBS оживает
  // при каждом переподключении вещателя, а паузой он гаснет и загорается
  // снова — без этой отметки подписчики получали бы «эфир начался»
  // по десять раз за вечер. Повтор — не раньше чем через два часа.
  liveNotifiedAt: {
    type: Date,
    default: null
  },
