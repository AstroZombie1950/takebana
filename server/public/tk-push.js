...
(function () {
  ...
  // телефоне подписка остаётся от прежнего владельца устройства, — и язык:
  // сервер берёт его из cookie запроса (routes/push.js).
  function sync() {
  ...
  // Переключили язык — пуши должны заговорить на нём сразу. Язык подписки
  // ставится на сервере из cookie, а страница при переключении не
  // перезагружается (public/tk-i18n.js): без этой строки устройство до
  // следующего захода получало уведомления на прежнем языке. Проверено
  // 23.09: сайт на английском, «в эфире» пришло по-русски.
  document.addEventListener('tk:lang', function () { sync(); });
})();
